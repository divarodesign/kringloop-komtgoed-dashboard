...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_quote") {
      ...
      const totalAmount = lines.filter((l: any) => l.Number > 0).reduce((s: number, l: any) => s + l.Number * l.PriceExcl * BTW_RATE, 0);
    ...
    if (action === "create_invoice") {
      ...
      const totalAmount = lines.reduce((s: number, l: any) => s + l.Number * l.PriceExcl * BTW_RATE, 0);
    ...
    if (action === "create_invoice_custom") {
      ...
      const invoiceNumber = invoiceResult.invoice?.InvoiceCode || null;
      const totalAmount = wefactLines.reduce((s: number, l: any) => s + l.Number * l.PriceExcl * BTW_RATE, 0);
